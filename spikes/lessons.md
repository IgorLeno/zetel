...
## SPEC-000/005 — Falha de gate por contencao de CPU, nao por teste (2026-08-04)

[2026-08-04] Context: `agentctl task validate` de SPEC-000 tarefa 005 gravou `validation: FAIL` com `pnpm test:ci` em `exit_code: 1`. Nenhum teste falhou no relatorio; o erro era `[vitest-worker]: Timeout calling "onTaskUpdate"`, um timeout de RPC entre o pool de workers do Vitest e o processo principal. O comportamento tambem aparecia no commit-base `4f826d3`, sem as mudancas da tarefa. A maquina rodava Vivaldi e Cursor pesados no momento.
Mistake: tratar exit code nao-zero de gate amplo como sinal de defeito do codigo antes de separar falha de asserção de falha de infraestrutura do runner. O caminho tentador era mexer em `vitest.config.ts` (`poolOptions`, `testTimeout`, `teardownTimeout`) — o que teria mascarado uma flutuacao de ambiente como se fosse requisito do projeto, e deixado config frouxa permanente no repositorio.
Rule: antes de alterar config de runner ou abrir waiver por gate vermelho, classificar a falha. Se o relatorio mostra **zero testes falhando** e o erro e de transporte do runner (`Timeout calling "onTaskUpdate"`, `Channel closed`, `worker exited unexpectedly`), tratar como ambiental: medir carga (`uptime`, `nproc`, `ps -eo pcpu`), liberar CPU e reexecutar o gate **sem nenhuma mudanca de codigo ou config**. Reproduzir no commit-base separa regressao de flutuacao. Só apos a repeticao em ambiente ocioso continuar vermelha e que se discute estabilizacao do gate (tarefa propria) ou waiver humano explicito — nunca waiver criado pelo agente por conta propria.
