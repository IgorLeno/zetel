...
[2026-06-03] Context: even within a single turn (no `cancel()`), a 5-sentence response could produce 5 simultaneous blobs ("media ativa" in DevTools), all playing at once.
Mistake: `finish()` resolved the serial chain promise but never called `audio.pause()`. When `audio.play()` was rejected by the browser (rapid blob succession, autoplay quirks), the chain advanced to the next sentence while leaving the prior `audio` element alive — browsers can start playback after a rejected `play()` promise, so the old audio sounded in parallel with the new one.
