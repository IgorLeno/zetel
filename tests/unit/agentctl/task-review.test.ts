...
describe('agentctl task review', () => {
  ...
  it('aggregates PASS, blocks independence/schema failures, and integrates with close', () => {
    ...
    expect(selfReview.stderr).toMatch(/guard:\s*review-aggregate|Writer/i);
    ...
    expect(samePkg.stderr).toMatch(/package_id duplicado|guard:\s*review-aggregate/i);
    ...
    expect(sameRun.stderr).toMatch(/review_run_id duplicado|guard:\s*review-aggregate/i);
  ...
  it('rejects writer self-review when reviews_requested is 1', () => {
    ...
    expect(agg.stderr).toMatch(/guard:\s*review-aggregate|autoaprovar|Writer/i);
  }, timeout);
